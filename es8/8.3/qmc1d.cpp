...
{
	if (argc!=2){
		cerr << "Program "<< argv[0] << " requires parameter <inputfile>"<<endl;
		exit(EXIT_FAILURE);
	}
...
void prepare_directories(){
	...
	if(PIGS){
		outputdir = basedir+"PIGS/";
		fs::create_directory(outputdir);
	}else{
		outputdir = basedir+"PIMC/";
		fs::create_directory(outputdir);
	}
}
...
{
	if (wavefunction_type=="flat"){
		return 1.0;
	}else if(wavefunction_type=="bumps"){
		double mu(0.78), sigma(0.61);
		return exp(-0.5 * pow((x-mu) / sigma, 2)) + exp(-0.5 * pow((x+mu) / sigma, 2));
	}else{
		cerr << "Invalid wavefunction type in VWF." << endl;
		exit(EXIT_FAILURE);
	}
}
...
double variationalWaveFunction_second(double x)
{
	if (wavefunction_type=="flat"){
		...
		return -0.5 * pow(sigma, -2) * ((plus + minus) +pow(sigma, -2) * (pow(x - mu, 2) * minus + pow(x + mu, 2) * plus));
	}else{
		cerr << "Invalid wavefunction type in VWF." << endl;
		exit(EXIT_FAILURE);
	}
...
{
	string filename = outputdir+"potential_"+wavefunction_type+".dat";
...
{
	string filename = outputdir+"kinetic_"+wavefunction_type+".dat";
	ofstream out(filename);
	for(int i=0;i<timeslices;i++)
	{
...
void finalizeHistogram()
{
	string filename;
	if (PIGS){
		filename = outputdir+"probability_"+wavefunction_type+".dat";
	}else{
		string s = to_string(temperature).substr(0,4);
		//string s = to_string(imaginaryTimePropagation).substr(0,4);
		filename = outputdir+"probability_"+s+".dat";
	}
	ofstream out(filename);
...
void readInput(const string & filename)
{
	...
	input_file >> string_away >> wavefunction_type;
