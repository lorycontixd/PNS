...
class BaseError : public std::exception {
    /* Base class for custom exception:
        ...
        -> msg (string): full custom message for the exception
        -> location (string): location of the message in the source code
        -> error (string): description of the error
        -> now (string): current date and time

        Custom exceptions are printed through the ```what()``` method, inherited from the std::exception class.
        The get_name and get_location methods return the name of the exception and the location of the error
    */
    private:
    protected:
        string name;
	...
	public:
        ...
        void update(){
            /*
            Sets the values for all the class variables and formats the error message.
            */
        ...
        void parse_name(){
            /*
            Writes the name of the error based on the class name
            */
            name = typeid(*this).name();
            string newname = "";
            for (char i : name){
...
class RepetitionError : public IndividualCheckFailure {
    private:
    protected:
        int value;
    public:
        RepetitionError(string _location, int _value) : IndividualCheckFailure(_location,"Repetition") {
            _value = value;
            update();
            msg += to_string(_value);
        }
