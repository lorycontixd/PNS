...
void LoadSettings(string configfile){
        ...
        settings::database_name = reader.Get("settings.database","name","results");
        settings::database_removeprevious = general::conversion::to_bool(reader.Get("settings.database","remove","true"));
...
auto PrintSettings(){
    ...
    cout << "Database: "<< boolalpha << settings::database_active<<endl;
    cout << "Name: "<<settings::database_name<<endl;
}
...
void LoadDatabaseSettings(Database &db){
    if (settings::database_active){
        if (db.IsConnected()){
            ...
            vector<tuple<string,string,string>> myvec = {key,_ncities,_layout,_box,_metric_type,_ntemp,_beta_step,_iters};
            ...
            string sql = "INSERT INTO SETTINGS (ID, NCITIES,LAYOUT,BOX,METRIC,NTEMP,BETA_STEP,ITERATIONS) VALUES ("+to_string(count)+","+to_string(settings::ncities)+",'"+settings::layout+"',"+to_string(settings::box)+",'"+settings::metric_type+"',"+to_string(settings::ntemp)+","+to_string(settings::step)+","+to_string(settings::iterations)+");";
            db.insert(sql);
